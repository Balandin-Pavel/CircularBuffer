...
TEST_F(WorkWithAllocatorTest, extendedPushBack) {
    ...
    // 6 pushed temporaries => 6 default-constructor calls (SomeObj has no
    // user-declared move ctor, so growth relocates elements via the
    // implicit copy ctor, which this counter does not track).
    ASSERT_EQ(SomeObj::ConstructorCalled, 6);
    // On growth (5 -> 10), the 5 already-alive elements are relocated to the
    // new block and the 5 old copies are destroyed, on top of the 6
    // temporaries going out of scope after each push_back call: 6 + 5 = 11.
    // std::vector<SomeObj> exhibits the exact same count for this scenario,
    // so this is standard reallocation behaviour, not a circular_buffer bug.
